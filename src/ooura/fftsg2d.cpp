...
void ddct(int n, int isgn, float *a, int *ip, float *w)
{
    ...
    // printf("makewt-1 nw:%d\n", nw);
    nc = ip[1];
    if (n > nc) {
        nc = n;
        makect(nc, ip, w + nw);
    }
    // printf("makect-1 nc:%d\n", nc);
    if (isgn < 0) {
        xr = a[n - 1];
        for (j = n - 2; j >= 2; j -= 2) {
            a[j + 1] = a[j] - a[j - 1];
...
void rftbsub(int n, float *a, int nc, float *c)
{
    ...
    for (j = 2; j < m; j += 2) {
        k = n - j;
        kk += ks;
        // printf("some indexes nc:%d, nc-kk:%d, kk:%d, j:%d, k:%d\n", nc, nc-kk,kk,j,k);
        wkr = 0.5 - c[nc - kk];
        // printf("passed nc-kk\n");
        wki = c[kk];
        xr = a[j] - a[k];
        xi = a[j + 1] + a[k + 1];
        // printf("multiply something\n");
        yr = wkr * xr + wki * xi;
        yi = wkr * xi - wki * xr;
        // printf("multiplied something\n");
        a[j] -= yr;
        a[j + 1] -= yi;
        a[k] += yr;
        a[k + 1] -= yi;
    }
    // printf("rftbsub ended\n");
}
...
void cdft2d(int n1, int n2, int isgn, Block &a, float *t, 
    int *ip, float *w)
{
    void makewt(int nw, int *ip, float *w);
    void cdft(int n, int isgn, float *a, int *ip, float *w);
    void cdft2d_sub(int n1, int n2, int isgn, Block &a, float *t, 
        int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void xdft2d0_subth(int n1, int n2, int icr, int isgn, Block &a, 
        int *ip, float *w);
    void cdft2d_subth(int n1, int n2, int isgn, Block &a, float *t, 
        int *ip, float *w);
...
void rdft2d(int n1, int n2, int isgn, Block &a, float *t, 
    int *ip, float *w)
{
    void makewt(int nw, int *ip, float *w);
    void makect(int nc, int *ip, float *c);
    ...
    void cdft2d_sub(int n1, int n2, int isgn, Block &a, float *t, 
        int *ip, float *w);
    void rdft2d_sub(int n1, int n2, int isgn, Block &a);
#ifdef USE_FFT2D_THREADS
    void xdft2d0_subth(int n1, int n2, int icr, int isgn, Block &a, 
        int *ip, float *w);
    void cdft2d_subth(int n1, int n2, int isgn, Block &a, float *t, 
...
void rdft2dsort(int n1, int n2, int isgn, Block &a)
{
    int n1h, i;
    float x, y;
    ...
    int *ip, float *w)
{
    ...
    void makect(int nc, int *ip, float *c);
    void ddct(int n, int isgn, float *a, int *ip, float *w);
    void ddxt2d_sub(int n1, int n2, int ics, int isgn, Block &a, 
        float *t, int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void ddxt2d0_subth(int n1, int n2, int ics, int isgn, Block &a, 
        int *ip, float *w);
    void ddxt2d_subth(int n1, int n2, int ics, int isgn, Block &a, 
        float *t, int *ip, float *w);
#endif /* USE_FFT2D_THREADS */
    int n, nw, nc, itnull, nthread, nt, i;
...
void ddst2d(int n1, int n2, int isgn, Block &a, float *t, 
    int *ip, float *w)
{
    void makewt(int nw, int *ip, float *w);
    void makect(int nc, int *ip, float *c);
